...
local ExtensionUI = require("ArmorVariantManager_Core.ExtensionUI")
...
local function load_config_data(body_id)
    ...
    if loaded_data then
        local migrated = false -- 标记是否触发了数据补齐/升级
        
        -- 1. 基础大层级兜底
        if not loaded_data.presets then loaded_data.presets = {}; migrated = true end
        if not loaded_data.default_preset then loaded_data.default_preset = ""; migrated = true end
        if not loaded_data.groups then loaded_data.groups = {}; migrated = true end
        if not loaded_data.transform_type then loaded_data.transform_type = "hp"; migrated = true end
        ...
        -- 2. 补齐全新的"并行条件"深层数据
        if loaded_data.is_parallel == nil then loaded_data.is_parallel = false; migrated = true end
        if not loaded_data.parallel_settings then
            loaded_data.parallel_settings = {}
            migrated = true
        end
        if type(loaded_data.parallel_settings.hp) ~= "table" then
            loaded_data.parallel_settings.hp = { enabled = true, priority = 1 }
            migrated = true
        end
        if type(loaded_data.parallel_settings.weapon) ~= "table" then
            loaded_data.parallel_settings.weapon = { enabled = false, priority = 2 }
            migrated = true
        end
        
        -- 3. 核心变身规则（transform_rules）深层清洗与节点补齐
        if not loaded_data.transform_rules then 
            loaded_data.transform_rules = {} 
            migrated = true
        else
            for _, r in ipairs(loaded_data.transform_rules) do
                -- 旧版单预设格式自动迁移至多 Targets 数组
                ...
                -- 【核心修复】清洗所有缺少的扩展节点字段，杜绝 UI 渲染层发生 nil 报错
                if r.targets == nil then r.targets = {}; migrated = true end
                if r.condition_delay == nil then r.condition_delay = 0; migrated = true end
                if r.mode == nil then r.mode = 1; migrated = true end
                if r.duration == nil then r.duration = 0; migrated = true end
                if r.loop_inactive_time == nil then r.loop_inactive_time = 0; migrated = true end
                if r.loop_count == nil then r.loop_count = 0; migrated = true end
                if r.chain_loop_count == nil then r.chain_loop_count = 1; migrated = true end
                
                -- 链式多节点结构清洗
                if r.chain_nodes == nil then 
                    r.chain_nodes = { { duration = 1, targets = {} } }
                    migrated = true 
                else
                    for _, node in ipairs(r.chain_nodes) do
                        if node.targets == nil then node.targets = {}; migrated = true end
                        if node.duration == nil then node.duration = 1; migrated = true end
                    end
                end
            end
        end
        ...
        if loaded_data.groups then
            for gname, g in pairs(loaded_data.groups) do
                if g.transform_rules and #g.transform_rules > 0 then
                    for _, old_r in ipairs(g.transform_rules) do
                        if old_r.preset and type(old_r.preset) == "string" then
                            ...
                            if not found then
                                table.insert(loaded_data.transform_rules, {
                                    threshold = old_r.threshold,
                                    targets = { { group = gname, preset = old_r.preset } }
                                })
                            end
                            migrated = true
                        end
                ...
                if g.transform_rules ~= nil then
                    g.transform_rules = nil
                    migrated = true
                end
            end
        end
        
        -- 【关键步骤】如果检测到当前读取的文件是旧格式并触发了补齐，直接导出新 JSON 覆盖原文件！
        if migrated then
            json.dump_file(path, loaded_data)
        end
        ...
        if char_body_id then
            ...
            if active_overrides[char_body_id] then
                if char and sdk.is_managed_object(char) then
                    ...
                    local ok, err = pcall(apply_preset_to_character, char, final_overrides, true, force_apply)
                    if not ok then
                        -- 仅在调试模式下打印错误，避免刷屏
        ...
        local status, err = pcall(function()
            ...
            if character and sdk.is_managed_object(character) then
                ...
                if body_id then
                    ...
                    pcall(function()
                        preset_node_open = imgui.tree_node(T("presets_manager") .. " (" .. body_id .. ")")
                    end)
                    ...
                    if preset_node_open then
                        ...
                        local ui_status, ui_err = pcall(function()
                            ...
                            if current_group_name ~= "" then
                                ...
                                if group_data and group_data.mask then
                                    local preview_node_open = false
                                    pcall(function()
                                        preview_node_open = imgui.tree_node(T("materials") .. " in " .. current_group_name)
                                    end)
                                    
                                    if preview_node_open then
                ...
                local transform_ui_status, transform_ui_err = pcall(function()
                    transform_node_open = imgui.tree_node(T("transform_manager") .. " (" .. body_id .. ")")
                end)
                
                if transform_ui_status and transform_node_open then
                    local inner_status, inner_err = pcall(function()
                        if hp_reflection_initialized and (not hp_getter) then
                            ...
                            if current_config.is_parallel then
                                ...
                                if changed_hp_pri then
                                    local p = tonumber(val_hp_pri)
                                    if p then hp_set.priority = p; save_current_config_to_file(body_id) end
                                end
                                ...
                                if changed_wp_pri then
                                    local p = tonumber(val_wp_pri)
                                    if p then wp_set.priority = p; save_current_config_to_file(body_id) end
                                end
                                imgui.unindent(10)
                            end
                            imgui.separator()
                            ...
                            if show_hp then
                                ...
                                if imgui.button(T("add_node")) then
                                    table.insert(rules, ExtensionUI.get_default_rule())
                                    save_current_config_to_file(body_id)
                                end
                                imgui.separator()
                                ...
                                for i, rule in ipairs(rules) do
                                    ...
                                    imgui.spacing()
                                    if ExtensionUI.draw_rule_ui(rule, i, all_groups, all_groups_display, current_config, T) then
                                        save_current_config_to_file(body_id)
                                    end
                            ...
                            if show_weapon then
                                if TransformManager.is_weapon_module_initialized() and (not TransformManager.has_weapon_getter()) then
                                ...
                                for i, rule in ipairs(w_rules) do
                                    ...
                                    for j, target in ipairs(rule.targets) do
                                        ...
                                        imgui.same_line()
                                        if imgui.button(T("delete_condition") .. "##w_del_cond") then
                                            table.remove(rule.targets, j)
                                            save_current_config_to_file(body_id)
                                        end
                                    ...
                                    if imgui.button("+ " .. T("add_condition") .. "##w_" .. i) then
                    ...
                    if not inner_status then
                        imgui.text_colored("UI Error: " .. tostring(inner_err), 0xFF00FFFF) -- 换成了黄色
                    end
                ...
                pcall(function()
                    lang_node_open = imgui.tree_node(T("language"))
                end)
                ...
                pcall(function()
                    perf_node_open = imgui.tree_node(T("performance_settings"))
                end)
