...
local function save_global_settings() json.dump_file(global_config_path, global_config) end
...
local current_config = { default_preset = "", presets = {} }
local PART_INDEX_TO_NAME = { [0] = "helm", [1] = "body", [2] = "arm", [3] = "waist", [4] = "leg", [5] = "slinger" }
...
local function get_player_manager() return sdk.get_managed_singleton("app.PlayerManager") end
...
local function get_character_body_id(character)
    ...
    if game_obj_status_cache and game_obj_cache then cache_key = tostring(game_obj_cache) else cache_key = tostring(character) end

    local cached = body_id_cache[cache_key]
    local current_time = os.clock()
    local ttl = global_config.body_id_ttl or 1.0
    if cached and (current_time - cached.last_check < ttl) then return cached.id end
    ...
    if status and body_part then
        ...
        if name_status and name then result_id = name end
    end
        ...
        if game_obj_status and game_obj then
            ...
            if transform then
                ...
                if #candidates > 0 then
                    for _, name in ipairs(candidates) do if not string.find(name, "^ch00") and string.match(name, "2$") then result_id = name; break end end
                    if not result_id then for _, name in ipairs(candidates) do if not string.find(name, "^ch00") and string.match(name, "3$") then result_id = name; break end end end
                    if not result_id then for _, name in ipairs(candidates) do if not string.find(name, "^ch00") and string.match(name, "1$") then result_id = name; break end end end
                    if not result_id then
                        if has_non_ch00 then for _, name in ipairs(candidates) do if not string.find(name, "^ch00") then result_id = name; break end end
                        else result_id = candidates[1] end
                    end
                end
            end
            ...
            if not result_id then
                ...
                local special_names = { ["Pl000_00"] = true, ["SaveSelect_HunterXX"] = true, ["SaveSelect_HunterXY"] = true, ["GuildCard_HunterXX"] = true, ["GuildCard_HunterXY"] = true, ["Lobby_HunterXX"] = true, ["Lobby_HunterXY"] = true }
                if name and special_names[name] then
                    local transform = game_obj:call("get_Transform")
                    if transform and transform:call("get_Child") then result_id = name end
                end
            end
        end
    end
...
local scanner = { state = "IDLE", transforms = nil, count = 0, index = 1, last_scan_time = 0 }
...
local function update_cache_entry(char)
    ...
    else game_obj = char:call("get_GameObject") end
    ...
    if not body_id or not string.find(body_id, "^ch03") then return end
...
local function tick_scanner()
    ...
    if scanner.state == "IDLE" then
        if (current_time - scanner.last_scan_time > scan_interval) then
            local ttl = global_config.body_id_ttl or 1.0
            for k, v in pairs(body_id_cache) do if current_time - v.last_check > ttl * 2 then body_id_cache[k] = nil end end

            local scene_manager = sdk.get_native_singleton("via.SceneManager")
            local scene = nil
            if scene_manager then scene = sdk.call_native_func(scene_manager, sdk.find_type_definition("via.SceneManager"), "get_CurrentScene") end
            ...
            if scene then
                ...
                if method_cache.Scene_findComponents and type_cache.via_transform then
                    ...
                    if transforms then
                        ...
                        scanner.count = #scanner.transforms; scanner.index = 1; scanner.state = "PROCESSING"
                    else scanner.last_scan_time = current_time end
                else scanner.last_scan_time = current_time end
            else scanner.last_scan_time = current_time end
        end
    elseif scanner.state == "PROCESSING" then
        local batch_size = global_config.scanner_batch_size or 100
        local limit = math.min(scanner.index + batch_size - 1, scanner.count)
        
        for i = scanner.index, limit do
            local status, transform = pcall(function() local t = scanner.transforms[i]; if t and sdk.is_managed_object(t) then return t end return nil end)
            if status and transform then
                ...
                if ok and game_obj and sdk.is_managed_object(game_obj) then
                    ...
                    if name_ok and name then
                        if string.sub(name, 1, 2) == "Pl" then is_target = true
                        else
                            local special_names = { "SaveSelect_HunterXX", "SaveSelect_HunterXY", "GuildCard_HunterXX", "GuildCard_HunterXY", "Lobby_HunterXX", "Lobby_HunterXY" }
                            for _, s_name in ipairs(special_names) do if name == s_name then is_target = true; break end end
                        end
                    end
                    if is_target then
                        local char = nil
        ...
        if scanner.index > scanner.count then scanner.state = "IDLE"; scanner.transforms = nil; scanner.last_scan_time = os.clock() end
    end
end
...
local function get_all_characters()
                ...
                if player then
                    ...
                    if char and sdk.is_managed_object(char) then
                        ...
                        if game_obj_ok and game_obj and sdk.is_managed_object(game_obj) then
                            local draw_status, is_draw = pcall(function() return game_obj:call("get_Draw") end)
                            if draw_status and is_draw == false then else
                                local key = tostring(game_obj)
                                if not seen_objs[key] then
                                    local bid = get_character_body_id(char)
                                    if bid and string.find(bid, "^ch03") then table.insert(chars, char); seen_objs[key] = true end
                                end
                            end
                        end
                    end
    ...
    local current_time = os.clock()
    local cache_ttl = (global_config.scan_interval or 2.0) + CACHE_TTL_BUFFER
    ...
    for key, data in pairs(character_cache) do
        ...
        if data.char and sdk.is_managed_object(data.char) then
            ...
            if game_obj_ok and game_obj and sdk.is_managed_object(game_obj) then
                local draw_status, is_draw = pcall(function() return game_obj:call("get_Draw") end)
                if draw_status and is_draw == false then is_valid = false else is_valid = true end
            end
        end
        if is_valid and (current_time - data.last_seen <= cache_ttl) then
            if not seen_objs[key] then table.insert(chars, data.char); seen_objs[key] = true end
        else character_cache[key] = nil end
    end
    return chars
end
...
local function get_local_player_character()
    ...
    if not char then
        ...
        if #all_chars > 0 then
            ...
            if last_valid_local_player then
                for _, c in ipairs(all_chars) do if c == last_valid_local_player then char = c; found_last = true; break end end
            end
    ...
    if char then
        if sdk.is_managed_object(char) then last_valid_local_player = char; last_valid_local_player_time = current_time end
    else
        if last_valid_local_player and (current_time - last_valid_local_player_time <= PLAYER_PERSISTENCE_TIME) then
            if sdk.is_managed_object(last_valid_local_player) then char = last_valid_local_player else last_valid_local_player = nil end
        end
    end
    return char
end

local function get_body_id() return get_character_body_id(get_local_player_character()) end
local function get_config_path(body_id) if not body_id then return nil end return "ArmorVariantManager/" .. body_id .. ".json" end

local function update_preset_names_list()
    preset_names_list = {}
    local target_presets = nil
    if current_group_name == "" then if current_config and current_config.presets then target_presets = current_config.presets end
    else if current_config and current_config.groups and current_config.groups[current_group_name] then target_presets = current_config.groups[current_group_name].presets or {} else target_presets = {} end end
    
    if target_presets then
        for name, _ in pairs(target_presets) do table.insert(preset_names_list, name) end
        table.sort(preset_names_list)
    end
    
    local ctx_default = ""
    if current_group_name == "" then ctx_default = current_config and current_config.default_preset or ""
    else if current_config and current_config.groups and current_config.groups[current_group_name] then ctx_default = current_config.groups[current_group_name].default_preset or "" end end

    if ctx_default ~= "" then
        local found = false
        for i, name in ipairs(preset_names_list) do if name == ctx_default then selected_preset_index = i; found = true; break end end
        if not found then selected_preset_index = 1 end
    else selected_preset_index = 1 end
    if #preset_names_list == 0 then selected_preset_index = 1 elseif selected_preset_index > #preset_names_list then selected_preset_index = 1 end
end

local function update_group_names_list()
    group_names_list = {}
    if current_config and current_config.groups then
        for name, _ in pairs(current_config.groups) do table.insert(group_names_list, name) end
        table.sort(group_names_list)
    end
    if #group_names_list == 0 then selected_group_index = 1 elseif selected_group_index > #group_names_list then selected_group_index = 1 end
end
...
local function get_mesh_component_recursive(game_obj)
    ...
    if not type_mesh then type_mesh = get_type("via.render.Mesh"); if not type_mesh then return nil end end
    ...
    if transform then
        ...
        while child do
            local child_obj = child:call("get_GameObject")
            if child_obj then mesh = child_obj:call("getComponent(System.Type)", type_mesh:get_runtime_type()); if mesh then return mesh end end
            child = child:call("get_Next")
        end
    end
    return nil
                ...
                if child_obj then
                    ...
                    if name and string.find(name, "^ch") then
                        ...
                        if suffix_str then
                            ...
                            if last_digit == 1 then target_index = 2 elseif last_digit == 2 then target_index = 1 elseif last_digit == 3 then target_index = 0 elseif last_digit == 4 then target_index = 4 elseif last_digit == 5 then target_index = 3 elseif last_digit == 6 then target_index = 5 end
                            if target_index then
                                ...
                                if mesh then
                                    ...
                                    if parts_map[target_index] then
                                        ...
                                        if not string.find(old_name, "^ch00") and string.find(name, "^ch00") then should_replace = false
                                        elseif not string.find(old_name, "^ch00") and not string.find(name, "^ch00") then should_replace = false
                                        elseif string.find(old_name, "^ch00") and string.find(name, "^ch00") then should_replace = false end
                                    end
                                    if should_replace then parts_map[target_index] = { obj = child_obj, name = name } end
                                end
                            end
                        end
                    end
...
local function get_material_group_owner(part_index, mat_name)
    ...
    for g_name, g_data in pairs(current_config.groups) do if g_data and g_data.mask and g_data.mask[s_idx] and g_data.mask[s_idx][mat_name] then return g_name end end
    return nil
end
...
local function is_material_in_current_context(part_index, mat_name)
    ...
    if current_group_name == "" then return owner == nil else return owner == current_group_name end
end
...
local function apply_preset_to_character(character, preset_data, ignore_context, force_apply)
    ...
    if not type_mesh then type_mesh = get_type("via.render.Mesh"); if not type_mesh then return end end
                ...
                if mesh_component then
                    ...
                    if part_data.mesh_enabled ~= nil then
                        ...
                        if part_data.mesh_enabled == false then
                            if current_enabled ~= false then mesh_component:call("set_Enabled", false) end
                        elseif should_apply then
                            if current_enabled ~= true then mesh_component:call("set_Enabled", true) end
                        end
                    end
                        ...
                        for j = 0, mat_count - 1 do
                            ...
                            if ignore_context or is_material_in_current_context(i, mat_name) then
                                ...
                                if mat_enabled == false then
                                    if current_mat_enabled ~= false then mesh_component:call("setMaterialsEnable", j, false) end
                                elseif mat_enabled == true and should_apply then
                                    if current_mat_enabled ~= true then mesh_component:call("setMaterialsEnable", j, true) end
                                end
                            end
                        end
                    end
...
local function create_new_group(group_name, body_id)
    ...
    for k, v in pairs(pending_material_selections) do if next(v) then has_selection = true break end end
    ...
    local new_group = { mask = deep_copy_table(pending_material_selections), presets = {} }
        ...
        for _, preset_data in pairs(current_config.presets) do
            for part_idx_str, mats in pairs(new_group.mask) do
                if preset_data[part_idx_str] and preset_data[part_idx_str].materials then
                    ...
                    for m_name, _ in pairs(mats) do p_mats[m_name] = nil end
                end
            end
        end
    end
...
local function delete_group(group_name, body_id)
    ...
    if current_group_name == group_name then current_group_name = ""; selected_group_index = 1; update_preset_names_list() end
    update_group_names_list()
    save_current_config_to_file(body_id)
    return true
end
...
local function load_config_data(body_id)
    ...
    if loaded_data then
        ...
        if not loaded_data.weapon_transform_rules then loaded_data.weapon_transform_rules = { { state = "sheathed", targets = {} }, { state = "drawn", targets = {} } } end
        if loaded_data.is_parallel == nil then loaded_data.is_parallel = false end
        if not loaded_data.parallel_settings then
            loaded_data.parallel_settings = { hp = { enabled = true, priority = 1 }, weapon = { enabled = false, priority = 2 } }
        end
        ...
        if not loaded_data.transform_rules then loaded_data.transform_rules = {} 
            ...
            for _, r in ipairs(loaded_data.transform_rules) do
                if r.preset and type(r.preset) == "string" and not r.targets then r.targets = { { group = "", preset = r.preset } }; r.preset = nil; migrated = true end
            end
        end
        
        if loaded_data.groups then
            for gname, g in pairs(loaded_data.groups) do
                if g.transform_rules and #g.transform_rules > 0 then
                    for _, old_r in ipairs(g.transform_rules) do
                        if old_r.preset and type(old_r.preset) == "string" then
                            local found = false
                            for _, global_r in ipairs(loaded_data.transform_rules) do
                                if global_r.threshold == old_r.threshold then
                                    if not global_r.targets then global_r.targets = {} end
                                    table.insert(global_r.targets, { group = gname, preset = old_r.preset })
                                    found = true; break
                                end
                            end
                            if not found then table.insert(loaded_data.transform_rules, { threshold = old_r.threshold, targets = { { group = gname, preset = old_r.preset } } }) end
                            migrated = true
                        end
                    end
                end
                if g.transform_rules ~= nil then g.transform_rules = nil; migrated = true end
            end
        end
        
        if migrated then json.dump_file(path, loaded_data) end
...
local function merge_preset_into_overrides(body_id, preset_data)
    ...
    for p_idx, p_data in pairs(preset_data) do
        ...
        if p_data.mesh_enabled ~= nil then overrides[p_idx].mesh_enabled = p_data.mesh_enabled end
        if p_data.materials then
            if not overrides[p_idx].materials then overrides[p_idx].materials = {} end
            for mat_name, is_enabled in pairs(p_data.materials) do overrides[p_idx].materials[mat_name] = is_enabled end
        end
    end
end
...
local function merge_overrides(base_data, add_data)
    ...
    for p_idx, p_data in pairs(add_data) do
        ...
        if p_data.mesh_enabled ~= nil then result[p_idx].mesh_enabled = p_data.mesh_enabled end
        if p_data.materials then
            if not result[p_idx].materials then result[p_idx].materials = {} end
            for mat_name, is_enabled in pairs(p_data.materials) do result[p_idx].materials[mat_name] = is_enabled end
        end
    end
    return result
end
...
local function get_current_preset_data(preset_name)
    if current_group_name == "" then if current_config and current_config.presets then return current_config.presets[preset_name] end
    else if current_config and current_config.groups and current_config.groups[current_group_name] then return current_config.groups[current_group_name].presets[preset_name] end end
    return nil
end
...
local function apply_preset(preset_name)
    ...
    if current_body_id then merge_preset_into_overrides(current_body_id, preset_data); temp_applied_presets[current_body_id] = preset_name end
    local all_chars = get_all_characters()
    for _, char in ipairs(all_chars) do
        local char_body_id = get_character_body_id(char)
        if char_body_id and char_body_id == current_body_id then apply_preset_to_character(char, active_overrides[current_body_id], true, true) end
    end
end

local function load_body_config(body_id)
    if not body_id then return false end
    current_config = { default_preset = "", presets = {}, groups = {}, transform_rules = {} }
    local data = load_config_data(body_id)
    if data then current_config = data; loaded_configs[body_id] = current_config end
    update_group_names_list()
    update_preset_names_list()
    if data then apply_all_defaults(body_id); return true end
    return false
end
...
local function save_preset(preset_name, body_id)
    ...
    for i = 0, 5 do
        ...
        if part_obj then
            local mesh_component = get_mesh_component_recursive(part_obj)
            if mesh_component then
                local part_data = { mesh_enabled = mesh_component:call("get_Enabled"), materials = {} }
                local mat_count = mesh_component:call("get_MaterialNum")
                if mat_count then
                    for j = 0, mat_count - 1 do
                        local mat_name = mesh_component:call("getMaterialName", j)
                        if is_material_in_current_context(i, mat_name) then part_data.materials[mat_name] = mesh_component:call("getMaterialsEnable", j) end
                    end
                end
                if next(part_data.materials) or current_group_name == "" then new_preset_data[tostring(i)] = part_data end
            end
        end
    end
    ...
    if current_group_name == "" then
        ...
        if not current_config.groups then current_config.groups = {} end
        if not current_config.groups[current_group_name] then current_config.groups[current_group_name] = { presets = {}, mask = {} } end
        if not current_config.groups[current_group_name].presets then current_config.groups[current_group_name].presets = {} end
...
local function find_auto_preset(target_body_id)
    ...
    local search_patterns = { "reframework/data/ArmorVariantManager/.*\\.json", "reframework\\\\data\\\\ArmorVariantManager\\\\.*\\.json", "ArmorVariantManager/.*\\.json", "ArmorVariantManager\\\\.*\\.json", "data/ArmorVariantManager/.*\\.json" }
    local files = {}
    for _, pattern in ipairs(search_patterns) do
        local found = fs.glob(pattern)
        if found and #found > 0 then for _, f in ipairs(found) do table.insert(files, f) end end
    end
    ...
    for _, file in ipairs(files) do
        if not string.find(file, target_body_id) then
            ...
            if not s then data_prefix = "reframework/data/"; s, e = string.find(file, data_prefix) end
...
local function draw_mesh_toggle(game_object, label, body_id, part_index)
    ...
    if not type_mesh then type_mesh = get_type("via.render.Mesh"); if not type_mesh then imgui.text_colored(label .. " " .. T("type_loading"), 0xFF808080); return end end
    ...
    else imgui.text_colored(label .. " " .. T("no_mesh"), 0xFF808080) end
end
...
re.on_frame(function()
    ...
    for _, char in ipairs(all_chars) do
        ...
        if char_body_id then
            ...
            if not active_overrides[char_body_id] then apply_all_defaults(char_body_id) end
            if active_overrides[char_body_id] then
                if char and sdk.is_managed_object(char) then
                    ...
                    local hp_overrides, changed = TransformManager.apply_transform_rules(char_addr, config, char, final_overrides, merge_overrides)
                    final_overrides = hp_overrides
                    force_apply = changed
                    pcall(apply_preset_to_character, char, final_overrides, true, force_apply)
                end
            end
        end
    end
    ...
    if imgui.tree_node(T("mod_name")) then
        ...
        local status, err = pcall(function()
            ...
            if character and sdk.is_managed_object(character) then
                ...
                if body_id then
                    local preset_node_open = false
                    pcall(function() preset_node_open = imgui.tree_node(T("presets_manager") .. " (" .. body_id .. ")") end)
                    if preset_node_open then
                        local ui_status, ui_err = pcall(function()
                            ...
                            if current_group_name ~= "" then
                                for i, gname in ipairs(group_names_list) do if gname == current_group_name then current_group_combo_index = i + 1 break end end
                            end
                            ...
                            if imgui.begin_table("PresetsLayout", 2, 512) then
                                ...
                                else imgui.text_colored("[" .. T("no_presets") .. "]", 0xFF808080) end
                                ...
                                if #preset_names_list > 0 then
                                    local current_preset_name = preset_names_list[selected_preset_index]
                                    local ctx_default = (current_group_name == "") and current_config.default_preset or (current_config.groups[current_group_name] and current_config.groups[current_group_name].default_preset)
                                    if imgui.button(T("delete_preset")) then
                                        if current_group_name == "" then
                                            ...
                                            if current_config.groups[current_group_name] then
                                                ...
                                                if current_config.groups[current_group_name].default_preset == current_preset_name then current_config.groups[current_group_name].default_preset = "" end
                                            end
                                        end
                                        update_preset_names_list()
                                        save_current_config_to_file(body_id)
                                    ...
                                    if imgui.button(T("set_as_default")) then
                                        if current_group_name == "" then current_config.default_preset = current_preset_name
                                        else if current_config.groups[current_group_name] then current_config.groups[current_group_name].default_preset = current_preset_name end end
                                        save_current_config_to_file(body_id)
                                    end
                                    if ctx_default == current_preset_name then imgui.same_line(); imgui.text_colored(T("is_default"), 0xFF00FF00) end
                                end
                                ...
                                if imgui.button(T("save_preset") .. "##p_left") then
                                    if new_preset_name ~= "" then if save_preset(new_preset_name, body_id) then new_preset_name = ""; update_preset_names_list() end end
                                end

                                imgui.table_next_column()
                                if not is_selection_mode then
                                    if imgui.button(T("start_selection") .. "##right") then is_selection_mode = true; pending_material_selections = {} end
                                    if current_group_name ~= "" then
                                        imgui.same_line()
                                        if imgui.button(T("delete_group") .. "##right") then delete_group(current_group_name, body_id) end
                                    end
                                else
                                    ...
                                    if imgui.button(T("confirm_creation") .. "##gconfirm") then
                                        if new_group_name ~= "" and create_new_group(new_group_name, body_id) then
                                            current_group_name = new_group_name; new_group_name = ""; is_selection_mode = false; update_group_names_list(); update_preset_names_list()
                                        end
                                    end
                                    imgui.same_line()
                            ...
                            if not has_any_data and current_config.groups then
                                for _, g in pairs(current_config.groups) do if g.presets and next(g.presets) then has_any_data = true; break end end
                            end
                ...
                else imgui.text_colored(T("no_body_part"), 0xFF0000FF) end
                
                imgui.separator()
                
                -- === 变身管理区域 ===
                local transform_node_open = false
                pcall(function() transform_node_open = imgui.tree_node(T("transform_manager") .. " (" .. body_id .. ")") end)
                
                if transform_node_open then
                    local inner_status, inner_err = pcall(function()
                        imgui.separator()
                        
                        if not current_config.transform_type then current_config.transform_type = "hp" end
                        if current_config.is_parallel == nil then current_config.is_parallel = false end
                        if not current_config.parallel_settings then
                            current_config.parallel_settings = {
                        ...
                        if not current_config.is_parallel then
                            local c_type_idx = current_config.transform_type == "hp" and 1 or 2
                            local c_type_list = { T("condition_hp"), T("condition_weapon") }
                            local c_changed, c_val = imgui.combo(T("transform_condition_type"), c_type_idx, c_type_list)
                            if c_changed then
                                current_config.transform_type = c_val == 1 and "hp" or "weapon"
                                save_current_config_to_file(body_id)
                            end
                        end
                        ...
                        if current_config.is_parallel then
                            imgui.indent(10)
                            local hp_set = current_config.parallel_settings.hp
                            local changed_hp_en, val_hp_en = imgui.checkbox(T("enable") .. " " .. T("condition_hp"), hp_set.enabled)
                            if changed_hp_en then hp_set.enabled = val_hp_en; save_current_config_to_file(body_id) end
                            imgui.same_line()
                            imgui.set_next_item_width(80)
                            local changed_hp_pri, val_hp_pri = imgui.input_text(T("priority") .. "##hp", tostring(hp_set.priority))
                            if changed_hp_pri then local p = tonumber(val_hp_pri); if p then hp_set.priority = p; save_current_config_to_file(body_id) end end
                            
                            local wp_set = current_config.parallel_settings.weapon
                            local changed_wp_en, val_wp_en = imgui.checkbox(T("enable") .. " " .. T("condition_weapon"), wp_set.enabled)
                            if changed_wp_en then wp_set.enabled = val_wp_en; save_current_config_to_file(body_id) end
                            imgui.same_line()
                            imgui.set_next_item_width(80)
                            local changed_wp_pri, val_wp_pri = imgui.input_text(T("priority") .. "##wp", tostring(wp_set.priority))
                            if changed_wp_pri then local p = tonumber(val_wp_pri); if p then wp_set.priority = p; save_current_config_to_file(body_id) end end
                        ...
                        if current_config.groups then
                            for gname, _ in pairs(current_config.groups) do table.insert(all_groups, gname); table.insert(all_groups_display, gname) end
                        end
                        
                        local show_hp = current_config.is_parallel and current_config.parallel_settings.hp.enabled or (not current_config.is_parallel and current_config.transform_type == "hp")
                        local show_weapon = current_config.is_parallel and current_config.parallel_settings.weapon.enabled or (not current_config.is_parallel and current_config.transform_type == "weapon")
                        ...
                        if show_hp then
                            ...
                            if imgui.button(T("add_node")) then
                                -- 【修复体验】：默认新增节点阈值改为50，防止100%满血秒触发锁死
                                table.insert(rules, { threshold = 50, targets = {}, trigger_on_damage = false, duration = 0, keep_until_state_ends = false })
                                save_current_config_to_file(body_id)
                            end
                            imgui.separator()
                            ...
                            for i, rule in ipairs(rules) do
                                ...
                                if c_dmg then rule.trigger_on_damage = v_dmg; save_current_config_to_file(body_id) end
                                imgui.same_line()
                                ...
                                if not rule.trigger_on_damage then
                                    ...
                                    if c_t then
                                        ...
                                        if num then if num < 1 then num = 1 end; if num > 100 then num = 100 end; rule.threshold = num; save_current_config_to_file(body_id) end
                                    end
                                    imgui.same_line()
                                end
                                
                                if imgui.button(T("delete_node") .. "##" .. i) then table.remove(rules, i); save_current_config_to_file(body_id) end
                                
                                imgui.spacing()
                                if not rule.duration then rule.duration = 0 end
                                imgui.set_next_item_width(120)
                                local c_dur, v_dur_str = imgui.input_text(T("duration") .. "##" .. i, tostring(rule.duration))
                                if c_dur then
                                    local num = tonumber(v_dur_str)
                                    if num then if num < 0 then num = 0 end; rule.duration = num; save_current_config_to_file(body_id) end
                                end
                                imgui.same_line()
                                ...
                                if not rule.trigger_on_damage then
                                    ...
                                    if c_keep then rule.keep_until_state_ends = v_keep; save_current_config_to_file(body_id) end
                                end
                                
                                imgui.indent(20)
                                if not rule.targets then rule.targets = {} end
                                for j, target in ipairs(rule.targets) do
                                    imgui.push_id("target_" .. j)
                                    local g_idx = 1
                                    ...
                                    if c_g then target.group = all_groups[v_g]; target.preset = ""; save_current_config_to_file(body_id) end
                                    imgui.same_line()
                                    ...
                                    if c_p and target_presets[v_p] ~= "None" then target.preset = target_presets[v_p]; save_current_config_to_file(body_id) end
                                    imgui.same_line()
                                    if imgui.button(T("delete_condition") .. "##del_cond") then table.remove(rule.targets, j); save_current_config_to_file(body_id) end
                                    imgui.pop_id()
                                end
                                if imgui.button("+ " .. T("add_condition") .. "##" .. i) then table.insert(rule.targets, { group = "", preset = "" }); save_current_config_to_file(body_id) end
                                imgui.unindent(20)
                                imgui.separator()
                                imgui.pop_id()
                            end
                        ...
                        if show_weapon then
                            ...
                            if not current_config.weapon_transform_rules then 
                                current_config.weapon_transform_rules = { { state = "sheathed", targets = {} }, { state = "drawn", targets = {} } }
                            end
                            local w_rules = current_config.weapon_transform_rules
                            
                            for i, rule in ipairs(w_rules) do
                                imgui.push_id("w_transform_rule_" .. i)
                                imgui.spacing()
                                local state_name = rule.state == "sheathed" and T("weapon_sheathed") or T("weapon_drawn")
                                imgui.text(state_name)
                                imgui.indent(20)
                                if not rule.targets then rule.targets = {} end
                                for j, target in ipairs(rule.targets) do
                                    imgui.push_id("w_target_" .. j)
                                    local g_idx = 1
                                    for idx, g in ipairs(all_groups) do if g == (target.group or "") then g_idx = idx break end end
                                    imgui.set_next_item_width(120)
                                    local c_g, v_g = imgui.combo("##w_group", g_idx, all_groups_display)
                                    if c_g then target.group = all_groups[v_g]; target.preset = ""; save_current_config_to_file(body_id) end
                                    imgui.same_line()
                                    local target_presets = {}
                                    if target.group == "" or target.group == nil then
                                    ...
                                    if #target_presets == 0 then table.insert(target_presets, "None") end
                                    imgui.set_next_item_width(150)
                                    local c_p, v_p = imgui.combo("##w_preset", p_idx, target_presets)
                                    if c_p and target_presets[v_p] ~= "None" then target.preset = target_presets[v_p]; save_current_config_to_file(body_id) end
                                    imgui.same_line()
                                    if imgui.button(T("delete_condition") .. "##w_del_cond") then table.remove(rule.targets, j); save_current_config_to_file(body_id) end
                                    imgui.pop_id()
                                end
                                if imgui.button("+ " .. T("add_condition") .. "##w_" .. i) then table.insert(rule.targets, { group = "", preset = "" }); save_current_config_to_file(body_id) end
                                imgui.unindent(20)
                                imgui.separator()
                                imgui.pop_id()
                            end
                ...
                local armor_parts = { [0] = T("helm"), [1] = T("body"), [2] = T("arm"), [3] = T("waist"), [4] = T("leg"), [5] = T("slinger") }
                ...
                if imgui.tree_node(T("armor_parts")) then
                    local inner_status, inner_err = pcall(function()
                        for i = 0, 5 do
                            ...
                            else imgui.text_colored(part_name .. " " .. T("not_equipped"), 0xFF808080) end
                        end
                    end)
                    pcall(imgui.tree_pop)
                end
                ...
                imgui.separator()
                local lang_node_open = false
                pcall(function() lang_node_open = imgui.tree_node(T("language")) end)
                if lang_node_open then
                    pcall(function()
                        local is_en = global_config.language == "en"
                        local changed_en, new_en = imgui.checkbox("English", is_en)
                        if changed_en and new_en then global_config.language = "en"; save_global_settings() end
                        imgui.same_line()
                        local is_zh = global_config.language == "zh"
                        local changed_zh, new_zh = imgui.checkbox("中文", is_zh)
                        if changed_zh and new_zh then global_config.language = "zh"; save_global_settings() end
                    end)
                    pcall(imgui.tree_pop)
                end
                
                imgui.separator()
                local perf_node_open = false
                pcall(function() perf_node_open = imgui.tree_node(T("performance_settings")) end)
                if perf_node_open then
                    pcall(function()
                        imgui.text(T("performance_desc")); imgui.spacing()
                        local changed_si, val_si = imgui.slider_float(T("scan_interval"), global_config.scan_interval, 0.1, 5.0)
                        if changed_si then global_config.scan_interval = val_si; save_global_settings() end
                        local changed_ttl, val_ttl = imgui.slider_float(T("refresh_interval"), global_config.body_id_ttl, 0.1, 10.0)
                        if changed_ttl then global_config.body_id_ttl = val_ttl; save_global_settings() end
                        local changed_bs, val_bs = imgui.slider_int(T("scanner_batch_size"), global_config.scanner_batch_size, 10, 1000)
                        if changed_bs then global_config.scanner_batch_size = val_bs; save_global_settings() end
                    end)
                    pcall(imgui.tree_pop)
                end

            else imgui.text_colored(T("waiting_for_player"), 0xFF0000FF) end
        end)
        
        if not status then imgui.text_colored(T("lua_error") .. tostring(err), 0xFF0000FF) end
        imgui.tree_pop()
    end
end)