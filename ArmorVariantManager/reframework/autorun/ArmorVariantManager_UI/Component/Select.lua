...
function Select.new(d2d_api, colors, fonts)
    ...
    -- 统一计算弹出层范围，字段点击阶段和最终绘制阶段必须使用同一坐标。
    function component:get_popup_rect(request)
        local row_h = 30
        local max_visible = math.max(1, math.min(request.max_visible, #request.values))
        local popup_h = max_visible * row_h + 8
        local popup_y = request.y + request.h + 4
        local _, surface_h = d2d_api.surface_size()
        if surface_h and popup_y + popup_h > surface_h - 8 then popup_y = request.y - popup_h - 4 end
        return request.x, popup_y, request.w, popup_h, row_h, max_visible
    end

    -- 弹出层位于所有字段上层；先消费其范围内的点击，避免输入穿透到下方选择器。
    function component:is_click_in_open_popup(mx, my)
        local request = self.active_request
        if not request or self.open_key ~= request.key or not Runtime.is_mouse_clicked() then return false end
        local popup_x, popup_y, popup_w, popup_h = self:get_popup_rect(request)
        return Runtime.point_in_rect(mx, my, popup_x, popup_y, popup_w, popup_h)
    end
    ...
    function component:draw(options)
        ...
        local popup_consumes_click = self:is_click_in_open_popup(mx, my)
        ...
        if not popup_consumes_click and hovered and Runtime.is_mouse_clicked() then
    ...
    function component:draw_popup()
        ...
        local mx, my = Runtime.mouse_position()
        local popup_x, popup_y, popup_w, popup_h, row_h, max_visible = self:get_popup_rect(request)
        d2d_api.fill_rounded_rect(popup_x, popup_y, popup_w, popup_h, 5, 5, colors.panel)
        d2d_api.outline_rect(popup_x, popup_y, popup_w, popup_h, 1, colors.accent)
        local start_index = 1
        local end_index = math.min(#values, max_visible)
        for index = start_index, end_index do
            local row_y = popup_y + 4 + (index - start_index) * row_h
            local hovered = Runtime.point_in_rect(mx, my, popup_x, row_y, popup_w, row_h)
            if hovered then d2d_api.fill_rect(popup_x + 2, row_y, popup_w - 4, row_h, colors.accent_dark) end
            local text_x, text_y = Runtime.center_text(fonts.small, request.labels[index],
                popup_x + 10, row_y, popup_w - 20, row_h)
        ...
        local inside_popup = Runtime.point_in_rect(mx, my, popup_x, popup_y, popup_w, popup_h)
