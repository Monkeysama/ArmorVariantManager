...
local Button = require("ArmorVariantManager_Core.Refd2d.Component.Button")
local Checkbox = require("ArmorVariantManager_Core.Refd2d.Component.Checkbox")
local Panel = require("ArmorVariantManager_Core.Refd2d.Component.Panel")
local List = require("ArmorVariantManager_Core.Refd2d.Component.List")
local Window = require("ArmorVariantManager_Core.Refd2d.Component.Window")
local Slider = require("ArmorVariantManager_Core.Refd2d.Component.Slider")
local Tag = require("ArmorVariantManager_Core.Refd2d.Component.Tag")
local Input = require("ArmorVariantManager_Core.Refd2d.Component.Input")
local InputNumber = require("ArmorVariantManager_Core.Refd2d.Component.InputNumber")
local Select = require("ArmorVariantManager_Core.Refd2d.Component.Select")
local InputBlocker = require("ArmorVariantManager_Core.Refd2d.InputBlocker")
local NativeTextInput = require("ArmorVariantManager_Core.Refd2d.NativeTextInput")
...
-- 新 UI 统一间距规范：外层 18px、板块之间 12px、板块内部 12px，列表行距 6px。
local LAYOUT = {
    content_inset = 18,
    panel_gap = 12,
    panel_inset = 12,
...
-- 创建 D2D 新 UI。deps 负责连接装备管理器的状态和操作，避免 UI 模块依赖业务局部变量。
function Refd2dUI.new(deps)
    local self = {
        deps = deps,
        available = d2d ~= nil,
        ready = false,
    ...
    setmetatable(self, { __index = Refd2dUI })
...
function Refd2dUI:T(key)
    if self.translate then return self.translate(key) end
    return key
end

-- 格式化本地化文本，兼容旧字典和不带格式参数的键。
function Refd2dUI:TF(key, ...)
    local value = self:T(key)
    if select("#", ...) == 0 then return value end
    return string.format(value, ...)
end

-- 将常用虚拟键转换为材质搜索字符，材质名称主要由英文、数字和下划线组成。
function Refd2dUI:search_key_character(key, force_lowercase)
...
-- 维护 Caps Lock 按下沿，供普通键盘输入计算英文字母大小写。
function Refd2dUI:update_caps_lock_state()
...
-- 从原始键盘状态中读取当前搜索按键，避免 get_first_key_down 与输入拦截状态冲突。
function Refd2dUI:get_search_key_down()
...
-- 读取带按键重复的普通键盘输入；不依赖 ImGui、IME 或剪贴板。
function Refd2dUI:input_key_pressed(key)
    local ok, down = pcall(function() return reframework:is_key_down(key) end)
...
-- 在更新阶段处理材质搜索输入，支持退格、按键重复和大小写。
function Refd2dUI:update_material_filter_input()
...
-- 在更新阶段处理新增预设名称输入，支持退格、按键重复和大小写。
function Refd2dUI:update_preset_name_input()
...
-- 释放输入状态，防止切换输入框后沿用上一个输入框的按键重复节拍。
function Refd2dUI:reset_text_input_state()
    self.input_key_down = {}
    self.input_repeat_at = {}
end

-- 将原生 IME 提交文本同步到当前 D2D 字段，输入框仍由 D2D 负责绘制。
function Refd2dUI:update_native_text_input()
...
-- 聚焦 D2D 文本框时启动原生 EDIT 代理；坐标仅在变更时同步给 DLL。
function Refd2dUI:activate_native_text_input(input_id, text, rect, reset)
    self.native_text_input:activate(input_id, text, rect, reset)
end

function Refd2dUI:sync_native_text_input(input_id, rect)
    self.native_text_input:sync_rect(input_id, rect)
end

function Refd2dUI:deactivate_native_text_input(input_id)
    self.native_text_input:deactivate(input_id)
end

-- 清理已经释放但本帧没有再次进入字符查询的按键，避免下次按下跳过首次延迟。
function Refd2dUI:clear_released_input_keys()
...
-- 更新分组名称输入，分组确认前只修改新 UI 的暂存值。
function Refd2dUI:update_group_name_input()
...
-- 更新右侧规则的数字输入，使用普通键盘路径，不依赖输入法代理。
function Refd2dUI:update_number_input()
...
-- 绘制可编辑的规则数字输入框，并返回经过范围限制的数值。
function Refd2dUI:draw_number_input(field_key, value, x, y, w, h, minimum, maximum, step)
...
-- 绘制分组操作区，创建时把材质勾选暂存到 pending_material_selections。
function Refd2dUI:draw_group_actions(x, y, w, h, context)
    local inner_x = x + LAYOUT.panel_inset
    local inner_w = w - LAYOUT.panel_inset * 2
    if not self.group_creation_mode then
        -- 分组操作按钮吸附在面板底部，和预设操作区保持独立的底部操作节奏。
    ...
    -- 创建模式的控件整体靠近底部，并加大模式标题与输入框之间的间距。
...
-- 将快捷键代码转换为界面可读名称。
function Refd2dUI:key_name(key)
...
-- 点击绑定按钮会产生鼠标按键；先等待其释放，避免被误保存为新快捷键。
function Refd2dUI:begin_key_binding()
    self.waiting_for_key = true
    self.waiting_for_key_release = false
end

-- 鼠标虚拟键不允许作为弹窗快捷键，避免 UI 点击改变配置。
function Refd2dUI:is_bindable_key(key)
    return type(key) == "number" and not (key >= 0x01 and key <= 0x06)
end

-- 每帧处理新 UI 快捷键，只在按下沿切换窗口可见状态。
function Refd2dUI:update()
    ...
    local update_ok, update_error = xpcall(function()
        -- 分组增删会重建配置和列表，必须在 on_frame 执行，不能在 D2D 绘制回调中直接修改。
        ...
        -- 快捷键捕获必须在 on_frame 中执行，避免在 D2D 绘制回调中调用输入接口。
        if self.waiting_for_key then
            if not self.waiting_for_key_release then
                ...
                -- 鼠标刚释放的这一帧也跳过，避免 REFramework 仍返回上一次点击的虚拟键。
                return
            end
        ...
        -- 弹窗打开时清空 GameInputManager 的实际输入；D2D 仍使用原始鼠标状态。
...
-- 绘制旧 UI 顶部的新 UI 设置入口，并返回是否需要隐藏旧 UI 内容。
function Refd2dUI:draw_settings()
    local config = self.deps.config
    ...
    -- 未启用新 UI 时不显示快捷键绑定，并与下方旧 UI 主功能区分隔。
    if not config.new_ui_enabled then
        imgui.separator()
        return false
    end
    ...
    imgui.text(self:T("d2d_advanced_hint"))
    return true
end

-- 绘制分组和预设列表面板。
-- 根据当前上下文直接读取分组预设，避免依赖主脚本中会被重新赋值的列表引用。
-- 同时保留旧 JSON 的无 preset_order 结构，并兼容未被排序的新预设。
local function get_group_presets(context)
    ...
    if type(target_order) == "table" and #target_order > 0 then
        ...
        -- 旧 JSON 或手动编辑的 JSON 可能遗漏排序项，仍然显示这些预设。
        for name, _ in pairs(target_presets) do
            if not added[name] then table.insert(presets, name) end
        end
    else
...
-- 获取当前主列表或分组的默认预设名称，兼容旧 JSON 缺少 default_preset 的情况。
...
-- 绘制预设面板底部操作区：所有操作均复用主脚本的旧业务逻辑。
function Refd2dUI:draw_preset_actions(x, y, w, h, context, presets)
    ...
    local selected_name = presets[context.selected_preset_index]
    ...
    if selected_name then
        ...
        if self.button:draw(self:T("overwrite_preset"), x + LAYOUT.panel_inset,
            button_y, inner_width, 28, false, nil, self.fonts.tiny) then
...
function Refd2dUI:draw_library(x, y, w, h, context, preset_w)
    ...
    if show_restore_warning then
        ...
        -- 与旧 UI 使用同一条提示文案，强调色保持与材质选择模式一致。
    ...
    self.list:draw(presets, preset_x + LAYOUT.panel_inset, y, preset_w - LAYOUT.panel_inset * 2, h, {
        ...
        selected = function(_, index) return index == context.selected_preset_index end,
...
-- 绘制展开部位中的 Mesh 和材质控制项，显示状态统一使用勾选框表达。
-- 绘制展开部件内的搜索、批量控制、分组标签和可滚动材质列表。
function Refd2dUI:draw_part_content(x, y, w, h, context, part_index)
    ...
    if self.group_creation_mode then
        -- 分组选择期间不允许修改 Mesh 总开关，原位置显示当前选择模式。
        Runtime.text(self.d2d, self.fonts.small, self:T("selection_mode"),
    ...
    for _, material in ipairs(view.materials or {}) do
        if material.name then
            ...
            -- 创建分组时显示全部材质；普通分组不能重复占用，全球分组允许重叠。
...
-- 将当前按键转换为数字输入字符，支持整数、小数和负号。
function Refd2dUI:number_key_character(key)
    if type(key) ~= "number" then return nil end
    if key >= 0x30 and key <= 0x39 then return string.char(key) end
    if key == 0xBE or key == 0x6E then return "." end
...
-- 返回变身条件的显示名称，配置中的条件键保持旧 JSON 格式不变。
function Refd2dUI:transform_type_label(type_key)
...
-- 返回当前配置中可用于变身目标的分组列表，第一项为空字符串代表默认分组。
function Refd2dUI:get_transform_groups(context)
    local groups = { "" }
    for _, name in ipairs(context.group_names or {}) do table.insert(groups, name) end
    return groups
end

-- 返回目标分组的预设列表，兼容旧 JSON 缺少 preset_order 的情况。
function Refd2dUI:get_transform_presets(context, group_name)
...
-- 循环选择目标分组或预设，使用 D2D 按钮代替旧 UI 的下拉框。
function Refd2dUI:cycle_transform_target(target, context, direction)
...
-- 创建一个与旧 UI 相同字段的规则节点，只有用户点击新增时才写入配置。
function Refd2dUI:new_transform_rule(type_key)
...
-- 读取规则节点的简短状态名称，避免在窄面板中堆叠大量说明文字。
function Refd2dUI:transform_rule_label(type_key, rule)
...
-- 保存变身面板改动，统一走主脚本当前 Body 配置保存入口。
function Refd2dUI:save_transform(context)
    if self.deps.save_transform then self.deps.save_transform(context) end
end

-- 绘制变身规则的目标行，点击分组/预设按钮可循环切换目标。
function Refd2dUI:draw_transform_target(target, context, x, y, w, visible)
...
-- 绘制单个条件类型的规则列表，规则字段与旧 UI 保持一致。
function Refd2dUI:draw_transform_rules(type_key, config, context, x, y, w, view_top, view_bottom)
    local definition
    ...
    -- 只有完整落在可视区域内的行才绘制，避免滚动时半行越过面板边界。
    ...
    for index = #rules, 1, -1 do
        ...
        y = y + 32
        if type_key == "damage" then
            -- 受击规则保留旧 UI 的延迟、持续时间和循环参数，使用紧凑步进按钮编辑。
        ...
        if type_key == "damage" and rule.mode == 3 then
            ...
            for node_index = #rule.chain_nodes, 1, -1 do
                local node = rule.chain_nodes[node_index]
                ...
                for target_index = #target_list, 1, -1 do
...
-- 绘制全宽变身管理面板，左侧选择模式，右侧编辑条件和规则节点。
function Refd2dUI:draw_transform_panel(x, y, w, h, context)
    self.panel:draw(x, y, w, h)
    ...
    if not config.is_parallel then
        ...
        if self.deps.get_transform_state then
            local state = self.deps.get_transform_state(selected_type, context.character)
            if state ~= nil then Runtime.text(self.d2d, self.fonts.tiny,
                self:T("current_state") .. ": " .. tostring(state), left_x,
                mode_y + 132, COLORS.muted) end
        end
    else
    ...
    -- 条件面板同样按完整行裁剪，避免标题和按钮绘制到上下边界之外。
    local function visible(top, bottom) return top >= view_top and bottom <= view_bottom end
    if not config.is_parallel then
        if visible(cursor_y, cursor_y + 24) then Runtime.text(self.d2d, self.fonts.tiny,
            self:transform_type_label(selected_type), right_x, cursor_y, COLORS.accent) end
        ...
        for _, definition in ipairs(TRANSFORM_TYPES) do
            ...
            if setting and setting.enabled then
                if visible(cursor_y, cursor_y + 24) then
                    Runtime.text(self.d2d, self.fonts.tiny, self:transform_type_label(definition.key),
                        right_x, cursor_y, COLORS.accent)
                end
    ...
    -- cursor_y 已经包含当前偏移量，不能再次加上滚动偏移，否则最大范围会不断增长。
    ...
    -- 下拉选项最后绘制，确保不会被后续规则行覆盖。
    self.select:draw_popup()
end

-- 绘制右侧二级页签，部件和设置共用同一个内容区域。
function Refd2dUI:draw_right_tabs(x, y, w)
    ...
    if self.fonts.body then
        -- 部件管理与变身管理使用同一宽度；英文时按较长文案扩展，避免文字溢出。
    ...
    -- 底部页签右端与右侧内容面板右边界对齐，部件、变身管理、设置依次排列。
    local tabs_x = x + w - LAYOUT.content_inset - parts_width - transform_width - settings_width - tab_gap * 2
    local tabs_y = y
...
-- 按字体实际宽度绘制本地化说明文本，兼容中英文并避免超出设置面板。
function Refd2dUI:draw_wrapped_text(text, x, y, max_width, font, line_height, color)
    local line = ""
    local line_y = y
...
-- 绘制右侧设置页，整合旧 UI 的语言、性能和快捷键配置。
function Refd2dUI:draw_settings_panel(x, y, w, h)
    local config = self.deps.config
    self.panel:draw(x, y, w, h)
...
-- 绘制统一的装备部位手风琴面板，点击部位标题即可展开或收起内容。
function Refd2dUI:draw_parts(x, y, w, h, context)
    self.panel:draw(x, y, w, h)
    Runtime.text(self.d2d, self.fonts.small,
    ...
    if self.expanded_part ~= nil and self.expanded_part < part_count then
        -- 左侧已显示当前部件名称，右侧直接从 Mesh 控制项开始，避免重复标题。
...
function Refd2dUI:draw()
    ...
    local sw, sh = self.d2d.surface_size()
    if not sw or not sh or sw < 320 or sh < 240 then return end
    local mx, my = Runtime.mouse_position()
    ...
    panel_x, panel_y = self.window:update(mx, my, panel_x, panel_y, panel_w, panel_h, sw, sh, excluded)
    self.window_x, self.window_y = panel_x, panel_y
    -- 拖拽后立即按新面板坐标重算顶部控件，避免按钮滞后一帧再归位。
    ...
    self.d2d.fill_rect(0, 0, sw, sh, 0x66000000)
    ...
    -- 左侧分组/预设依赖角色数据，右侧设置页不依赖角色，应始终可访问。
    ...
    if self.right_panel ~= "transform" and replaceable_w > 80 then
    ...
    -- 绘制轻量级自绘箭头光标，放在所有控件之后，确保光标始终位于弹窗上层。
    -- 使用少量线段而不是每帧创建图片资源，避免额外的帧耗和资源抖动。
    local cursor_x, cursor_y = Runtime.mouse_position()
