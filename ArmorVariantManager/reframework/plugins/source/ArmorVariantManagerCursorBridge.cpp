...
std::filesystem::path g_text_request_path;
std::filesystem::path g_text_result_path;
...
BridgeState read_state() {
    BridgeState state;
    if (g_state_path.empty()) return state;
    std::ifstream file(g_state_path, std::ios::binary);
...
bool read_text_request(TextRequest& request) {
    if (g_text_request_path.empty()) return false;
    std::ifstream file(g_text_request_path, std::ios::binary);
    if (!file) return false;
    const std::string content((std::istreambuf_iterator<char>(file)),
        std::istreambuf_iterator<char>());
    TextRequest next;
    if (!read_json_bool(content, "active", next.active)) return false;
    if (!next.active) { request = next; return true; }
...
void publish_text_result(bool focused = true) {
    if (g_text_result_path.empty() || g_text_input_id.empty()) return;
    ...
    const auto temporary_path = g_text_result_path.wstring() + L".tmp";
    std::ofstream file(temporary_path, std::ios::binary | std::ios::trunc);
    if (!file) return;
    file.write(content.data(), static_cast<std::streamsize>(content.size()));
    file.close();
    MoveFileExW(temporary_path.c_str(), g_text_result_path.c_str(),
...
void apply_text_request(const TextRequest& request) {
    ...
    const bool new_session = request.session != g_text_session || request.input_id != g_text_input_id
...
void state_worker() {
    while (g_running.load(std::memory_order_acquire)) {
        apply_blocked_state(read_state().blocked);
        TextRequest request;
        if (read_text_request(request)) apply_text_request(request);
        MSG message{};
...
void initialize_bridge() {
    // 运行时通信文件与 GlobalSettings 分离，目录被手动清理后也能自动重建。
    const auto runtime_directory = get_reframework_directory() / L"data" / L"ArmorVariantManager" / L"Runtime";
    std::error_code create_directory_error;
    std::filesystem::create_directories(runtime_directory, create_directory_error);
    g_state_path = runtime_directory / L"D2DInputState.json";
    g_text_request_path = runtime_directory / L"D2DTextInputRequest.json";
    g_text_result_path = runtime_directory / L"D2DTextInputResult.json";
