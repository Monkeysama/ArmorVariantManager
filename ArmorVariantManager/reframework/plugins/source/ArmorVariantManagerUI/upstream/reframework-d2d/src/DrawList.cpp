...
void DrawList::CommandLock::push_transform(float x, float y, float scale) {
    Command cmd{};
    cmd.type = CommandType::PUSH_TRANSFORM;
    cmd.transform.x = x;
    cmd.transform.y = y;
...
void DrawList::CommandLock::pop_transform() {
    Command cmd{};
    cmd.type = CommandType::POP_TRANSFORM;
    commands.emplace_back(std::move(cmd));
}
