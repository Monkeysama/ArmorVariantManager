...
void on_ref_lua_state_created(lua_State* l) try {
    ...
    // 作用域变换用于整体 UI 缩放；未调用时保持原有单位矩阵行为。
    d2d["push_transform"] = [](float x, float y, float scale) {
        g_plugin->cmds->push_transform(x, y, std::max(0.01f, scale));
    };
    d2d["pop_transform"] = []() { g_plugin->cmds->pop_transform(); };
...
void on_ref_frame() try {
    ...
    g_plugin->d3d12->render(
        [](D2DPainter& d2d) {
            ...
            std::vector<D2D1_MATRIX_3X2_F> transforms{D2D1::Matrix3x2F::Identity()};
            d2d.context()->SetTransform(transforms.back());
            for (auto&& cmd : cmds_lock.commands) {
                switch (cmd.type) {
                case DrawList::CommandType::PUSH_TRANSFORM: {
                    const auto next = transforms.back()
                        * D2D1::Matrix3x2F::Scale(cmd.transform.scale, cmd.transform.scale)
                        * D2D1::Matrix3x2F::Translation(cmd.transform.x, cmd.transform.y);
                    transforms.push_back(next);
                    d2d.context()->SetTransform(next);
                    break;
                }
                case DrawList::CommandType::POP_TRANSFORM:
